// client/src/components/Message.js - VERSÃO MODIFICADA
...
// ... (Todo o código dos Ícones PlayIcon, PauseIcon, MicrophoneIcon e da função formatTime continua IGUAL) ...
...
// 1. Recebi os novos props 'playingAudioId' e 'setPlayingAudioId'
function Message({ message, playingAudioId, setPlayingAudioId }) {
  const waveformRef = useRef(null);
  const wavesurferRef = useRef(null);

  // --- MUDANÇA PARA O ÁUDIO ---
  // 2. REMOVI o 'useState' local de 'isPlaying'. A 'ChatWindow' agora controla isso.
  // const [isPlaying, setIsPlaying] = useState(false); // <-- REMOVIDO

  // 3. A 'verdade' vem de cima: este áudio está tocando?
  const isThisPlaying = message.id === playingAudioId;

  const [duration, setDuration] = useState('0:00');
  const [micColor, setMicColor] = useState('#0cd464');

  // Este useEffect SÓ CRIA o player de áudio
  useEffect(() => {
    if (message.type === 'audio' && waveformRef.current) {
      ...
      // --- MUDANÇA PARA O ÁUDIO ---
      // 4. Simplifiquei os 'listeners'.
      // Os 'listeners' de 'play' e 'pause' foram removidos.
      // O 'finish' é o mais importante: avisa o 'pai' que acabou.
      wavesurfer.on('finish', () => {
        setPlayingAudioId(null); // <-- Avisa o 'ChatWindow' que nenhum áudio toca mais
        wavesurfer.seekTo(0);
      });
    ...
    // --- MUDANÇA PARA O ÁUDIO ---
    // Adicionei setPlayingAudioId à lista de dependências
  }, [message.content, message.type, setPlayingAudioId]);

  // --- MUDANÇA PARA O ÁUDIO ---
  // 5. Criei um NOVO useEffect para REAGIR à mudança do 'isThisPlaying'
  // Este é o 'controle remoto' que o 'ChatWindow' usa
  useEffect(() => {
    if (wavesurferRef.current) {
      if (isThisPlaying) {
        ...
        // Isso vai ser chamado em TODOS os outros áudios quando um novo for selecionado
        wavesurferRef.current.pause();
        setMicColor('#0cd464');
      }
    }
  }, [isThisPlaying]); // O 'gatilho' é o nosso estado derivado

  // --- MUDANÇA PARA O ÁUDIO ---
  // 6. O 'handlePlayPause' agora controla o estado 'pai'
  const handlePlayPause = () => {
    if (isThisPlaying) {
      setPlayingAudioId(null); // Se está tocando, para
    } else {
      setPlayingAudioId(message.id); // Se está parado, toca (e para os outros)
    }
  };
        ...
        contentNode = (
          <div className="audio-bubble-custom">
            <button onClick={handlePlayPause} className="play-pause-btn">
              {/* --- MUDANÇA PARA O ÁUDIO --- */}
              {/* 7. O ícone agora depende de 'isThisPlaying' */}
